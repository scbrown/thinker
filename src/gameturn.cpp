...
void __cdecl turn_upkeep() {
    ...
    before *CurrentTurn is incremented forty-odd lines down. On entry *CurrentTurn
    is the turn that just finished, with all of its faction upkeep run, all of its
    observation records flushed, and its autosave already written by
    faction_upkeep. So the run stops holding exactly N complete turns rather
    ...
    this site over mod_auto_save, the tail of faction_upkeep, and the window
